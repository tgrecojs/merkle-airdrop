...
const addresses = ["0x77485F4Df482fC2abe4C787AF523b260f740C1FF",
...
/**
 * 
 * @description the createLayers function is responsible for creating the tree data structure. 
 * @param {Array} addresses The list of addresses that will be used when constructing the tree 
 * @returns 
 */
        ...
        createLayers(
          leaves.reduce((acc, _, i, arr) => {
            ...
            if (i % 2 === 0) {
              ...
              console.log('------------------------')
              console.log('@@@ hash1::', hash1, hash1.toString('utf8'))
              console.log('------------------------')
...
const sanitized = addresses.map(sanitizeAddress)
.map(trace('after sinitze'))
.map(sha256)
.map(trace('after hashing')) //?

const tree = createMerkleTree(sanitized)
tree //?
